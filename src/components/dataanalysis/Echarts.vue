<template>
  <div
    class="analysisCardStyle"
    id="analysisCardStyle"
  >
    <div
      id="echartsCard"
      v-loading="loading"
    >
      ...
      <div
        v-show="!tableVisible&&!tableSecVisible"
...
import DropFilterNumItem from "./drop/DropFilterNumItem";
...
export default {
  ...
  mounted() {
    Bus.$on("saveCharts", () => {
      ...
      let query = this.$post("/task/chart/save_chart", {
    ...
    Bus.$on("coldata", e => {
      ...
      if (this.xAxisItem.length > 0) {
        ...
        this.chartMethod = this.chartMethod.join(",");
        //发送求和请求
        let query = this.$post("/task/chart/result", {
          chart_id: this.chartId,
          data_set: this.dataSetId, //这是data_set_id
          chart_method: this.chartMethod,
          chart_type: 1,
    ...
    Bus.$on("chartMethod", (type, dropName) => {
      ...
      this.$post("/task/chart/result", {
        ...
        .then(r => {
          ...
          this.responseOriginData = r.data;
  ...
  methods: {
    ...
    optionToContent1(opt){
      let axisData = opt.xAxis[0].data;
      let series = opt.series;
      let table = `<template><el-table :data ="opt" style="width:100%"><div v-for="(item,key) in series[0]" :key=item.id></el-table></template>`;
      return table;
    ...
    optionToContent(opt) {
      ...
      let table = `<table border="1"class="table table-bordered table-striped table-hover" style="text-align:center  border-collapse:collapse;" border="1" "><tbody><tr>${tdHeads} </tr>`;
      ...
      this.option = {
        ...
        toolbox: {
          ...
          feature: {
            ...
            //   // yAxisIndex: "none"
            // },
            dataView: { readOnly: true, optionToContent: this.optionToContent,buttonColor:"#1d8fe1" },
            saveAsImage: {}
          }
        },
        calculable: true,
    ...
    pieDrawLine(pieType) {
      ...
      let allPieValue =this.seriesData[0]["data"];
      let restPieValue = allPieValue.slice(10);
      for (let i = 0; i < 10; i++) {
        let pieDataItem = {
          value: pieValue[i],
          name: this.Xdata[i]
      ...
      let sum = restPieValue.reduce( function(total,num){
        return total + num;
      },0);
      ...
      pieData.push(allPieDataItem);
      // console.log("pieData: ", pieData);
      // console.log("pieData: ", pieValue);
      // console.log("restpiedata:"  ,restPieValue);
      // console.log("allPie:",sum);
      ...
      this.option = {
        ...
        toolbox: {
          ...
          feature: {
            ...
            //   yAxisIndex: "none"
            // },
            dataView: { readOnly: false,optionToContent:this.optionToContent },
    ...
    sortClick(SortRadio, selectItem) {
      ...
      this.$post("/task/chart/result", {
        ...
        .then(r => {
          ...
          let str2 = str1.replace(/\["|\"]/g, "");
          let str3 = str2.replace(/\","/g, "·");
          this.responseData = r.data; //使得所有的数据都是排序的
