...
<script>
  ...
  export default {
    ...
    mounted() {
      vueBus.$on('yAixsFail', (e) => {
        this.dropCol.pop()
      })
      vueBus.$on('AxiosDataDragItem', (e) => {
        this.dataArray = e
        console.log(e)
      })
    },
    methods: {
      ...
      rowDrop: function (ev) {
        ...
        vueBus.$emit('rowdata', data)
        if (this.dropRow && this.dropCol) vueBus.$emit('featureConfigurationFlag', true)
      ...
      colDrop: function (ev) {
        ...
        vueBus.$emit('coldata', data)//给echarts组件发名字
        vueBus.$emit('featureConfiguration', data)//给功能配置发送消息
        if (this.dropRow && this.dropCol) vueBus.$emit('featureConfigurationFlag', true)
      ...
      colAxisDrop: function (ev) {
        ...
        vueBus.$emit('dropAxisCol', data)//给echarts组件发名字
      ...
      rowRemove(tag) {
        vueBus.$emit('rowMiningRemove', tag);
        this.dropRow.splice(this.dropRow.indexOf(tag), 1);
      },
      colRemove(tag) {
        vueBus.$emit('colMiningRemove', tag)
        this.dropCol.splice(this.dropCol.indexOf(tag), 1);
      },
      summationType() {//没用等删
        vueBus.$emit('summation', 'summationType')
      },
      addRowClick() {
        this.axisFlag = true;
