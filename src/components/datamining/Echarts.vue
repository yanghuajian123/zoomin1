<template>
  <div>
    <div
      class="box-card miningCardStyle"
      id="echartsCard"
    >
      ...
      >
        <el-tab-pane id="chartData">
          ...
          <img
            :src="clusterImg[0]"
            style="width:90%;height:90%;margin:45px;"
            v-show="axisClusterFlag"
          >
          <el-carousel
            v-show="carouselFlag"
...
import bg from "@/common/image/DataMining/chartBg.png";
const url = "http://120.79.146.91:8000";
...
export default {
  ...
  mounted() {
    ...
    vueBus.$on("getMiningDataSetId", e => {
      this.dataSetId = e;
    ...
    vueBus.$on("test_size", e => {
      this.test_size = e;
      //alert(this.test_size)
      this.drawModel();
    });
    vueBus.$on("mth_power", e => {
      this.mth_power = e;
      //alert(this.mth_power)
      this.drawModel();
    });
    vueBus.$on("error_type", e => {
    ...
    //3.监听标题改变(还没有PATCH)
    vueBus.$on("titleChange", e => {
      this.chartTitle = e;
    });

    //x,y轴的标题
    vueBus.$on("miningXAxis", e => {
      this.xAxisLabel = e;
      this.drawModel();
    });
    vueBus.$on("miningYAxis", e => {
      this.yAxisLabel = e;
      this.drawModel();
    });
  ...
  methods: {
    ...
    autoDivSize() {
      ...
      //DIV高度为浏览器窗口的高度
      document.getElementById("myChart").style.height =
        this.winHeight * 0.8 + "px";
      document.getElementById("myChart").style.width =
        this.winWidth * 0.68 + "px";
      document.getElementById("chartData").style.height =
        this.winHeight * 0.7 + "px";
      document.getElementById("chartData").style.width =
        this.winWidth * 0.67 + "px";
      document.getElementById("errorInfo").style.height =
        this.winHeight * 0.7 + "px";
      document.getElementById("errorInfo").style.width =
        this.winWidth * 0.67 + "px";
      document.getElementById("echartsCard").style.height =
        this.winHeight * 0.84 + "px";
      document.getElementById("echartsCard").style.width =
        this.winWidth * 0.68 + "px";
    ...
    drawModel() {
      ...
      //发送请求
      // setTimeout(() => {
      this.$post("/dataMining/regression/", 
      {
    ...
    drawCluster() {
      ...
      this.loading = true;
      this.$post("/dataMining/clustering/", {
        ...
        .then(r => {
          ...
          this.clusterImg[1] =
           url +
            r.data[2].slice(15, r.data[2].length);
          this.clusterImg[2] =
            url +
            r.data[3].slice(15, r.data[3].length);
          this.errorSumFlag = false;
          this.loading = false;
        })
        .catch(response => {
